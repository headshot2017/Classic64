...
l3:
            if (*itInstrs != NULL) {
                ...
                if (instrument->loaded == 0) {
					LOAD_SOUND(&(instrument->lowNotesSound));
					LOAD_SOUND(&(instrument->normalNotesSound));
					LOAD_SOUND(&(instrument->highNotesSound));
                    instrument->loaded = 1;
                }
            }
            itInstrs++;
